...
test_primary_and_secondmate_instruction_generation() {
  ...
  assert_grep 'NEVER pass `--yes` (or `-y`) to `no-mistakes axi run` or `'"$ROOT"'/bin/fm-nm-respond.sh`. It is banned fleet-wide.' "$ship" \
    "generated implementation brief does not prohibit silent ask-user auto-resolution"
  # shellcheck disable=SC2016 # Backticks are literal generated Markdown.
  assert_grep 'Ask-user gates must return to firstmate as `needs-decision`; the worker never answers its own finding.' "$ship" \
    "generated implementation brief does not route ask-user gates back to firstmate"
