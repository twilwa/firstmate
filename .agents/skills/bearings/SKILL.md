...
- When one authored option means "later", put `close: "defer"` and an explicit `until: "YYYY-MM-DD"` on that option so only that selection records the answer and dates the hold.
- Do not reserve a `defer` answer value: the option's close metadata carries the behavior, while the option value remains the captain-facing answer and only `reconcile` stays reserved.
