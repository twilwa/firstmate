...
Resolve the project's registered ship-branch prefix with `bin/fm-project-mode.sh --branch-prefix <project>` and pass it explicitly to the brief, ship spawn, and scout promotion as `--branch-prefix`; the default `fm/` needs no flag.
