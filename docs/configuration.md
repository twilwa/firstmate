...
FM_CODEX_WATCH_CHECKPOINT=180   # diagnostic-only timeout for fm-watch-checkpoint.sh; normal Codex primary supervision uses the Stop-owned park
FM_CODEX_PARK_RENEW_SECONDS=21600   # diagnostic/test override for the Codex park renewal; invalid or >= the native hook timeout falls back to one quarter of that timeout
