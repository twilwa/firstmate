...
`bin/fm-pr-risk.sh` owns the classification decision, while `bin/fm-pr-review.sh` owns the private data-relative ledger schema and the authenticated watcher shim.
