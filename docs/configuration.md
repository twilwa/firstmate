...
The run copies the brief once before reading it, and that one snapshot supplies the kind line, task sections, whole-brief fallback, and `brief_sha256`, so the receipt's hash names exactly the bytes the request was built from even if the file changes mid-run.
