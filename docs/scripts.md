...
| `fm-codex-stop-park.sh`  | Keep Codex's synchronous Stop hook attached to one watcher cycle and return its wake as a continuation |
| `fm-watch-checkpoint.sh` | Run one bounded foreground watcher checkpoint for diagnostics, recovering only a dead timed-out holder and refusing quiet success while its lock remains live |
...
| `fm-pr-merge.sh`         | Guard the reviewed-head GitHub handoff or a direct GitLab merge, record PR metadata, and refuse an outcome it cannot prove landed or queued |
