...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`, plus the project's registered `--forge`), scout, secondmate-charter, and Herdr-lab briefs, with Captain's intent and Firstmate spec subsections and a `--tests` scope declaration on ship/scout |
| [`fm-dod-lib.sh`](../bin/fm-dod-lib.sh) | Own ship/scout worker role scope, ship definitions of done, the task test-scope declaration, the named-head reachability gate on ship `done:` acceptance, and the no-mistakes `--intent` contract |
...
| `fm-promote.sh`          | Promote a scout task in place to a protected ship task with an explicit delivery mode, write the ship instructions carrying that mode's definition of done and the task's test scope, and supersede the task's brief so a later relaunch cannot revive stale scout delivery text |
