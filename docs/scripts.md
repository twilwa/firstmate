...
| `fm-pr-merge.sh`         | Require the reviewed-head GitHub handoff only when the repository policy opts in, or perform a direct GitLab merge; record PR metadata, refuse Gerrit submission, and refuse an outcome it cannot prove landed or queued |
