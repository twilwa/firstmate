...
`bin/fm-lint.sh` owns three canonical CI partitions, each running the same full source-aware ShellCheck analysis with one worker by default, pinned versions, workflow validation, and backend-purity checks.
The smaller root sets and sequential workers limit peak ShellCheck memory without dropping any checks.
