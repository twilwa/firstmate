...
Claude's slash-command completion popup can fill the usual tail below the composer, so when that read selects no composer the post-paste proof selects from a 200-row read of the same capture, where the bottom-most composer still wins.
