...
`bin/fm-procevent-lavish.sh answers` is one such adapter command; it reads only rows tagged `choice`, relays a card or option's declared close mode and required defer date, and can never let freeform captain prose forge a task id, mode, or date.
