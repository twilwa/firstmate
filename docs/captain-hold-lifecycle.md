...
The optional mode column carries a caller-declared outcome: `done` (default) completes the task, `release` lifts the hold so held work resumes, and `defer` requires a fifth YYYY-MM-DD field that records the answer and leaves the task held behind that date; any other value, a missing defer date, or a date on another mode is skipped.
...
`bin/fm-send.sh --resolve-key` is the chat channel: its status-log close for a key the status log still owns is owned by that script's header, and a key the status log no longer owns is resolved to a still-open captain-held task - the key as a task id, then the legacy derived identity - and fed as one keyed line; `--defer-until YYYY-MM-DD` selects the dated mode for those captain-held keys.
