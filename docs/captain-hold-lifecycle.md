...
Retries of an active hold preserve its hold-set timestamp, while re-holding released work starts a new timestamped lifecycle; a closed task is refused rather than reopened, and `--until` gates the hold behind a date through tasks-axi's date gate while recording no answer of its own.
