...
A deferral continues the call it answers rather than opening another, so it leaves the `Captain hold set:` stamp alone - a live and an already-expired hold keep one age basis instead of the expired one restarting the clock - and publishes only its `resolved ... deferred until <date>` parent line, never a fresh `needs-decision` for the question the captain just postponed.
