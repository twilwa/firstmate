...
Retries of an active hold preserve its hold-set timestamp, while re-holding released work starts a new timestamped lifecycle whose stamp is strictly later than the one its previous answer recorded; a closed task is refused rather than reopened, and `--until` gates the hold behind a date through tasks-axi's date gate while recording no answer of its own.

The `answer` subcommand records the captain's exact words and resolves the call in the same act: it closes a question-shaped call, `answer --release` frees a captain-gated work item to proceed without completing it, and `answer --defer-until YYYY-MM-DD` re-dates the call's existing captain hold through the same `tasks-axi hold --until` gate so the answered call remains held behind its date.
A recorded-answer defer requires a date strictly later than today's UTC calendar date: direct `answer --defer-until`, the keyed `defer` mode, and chat's `--defer-until` preflight refuse past and same-day values before recording or delivery, while bare `hold --until` continues to accept any valid calendar date.
A deferral continues the call it answers rather than opening another, so it leaves the `Captain hold set:` stamp alone - including preserving its absence on a legacy stampless hold - carries the hold's existing reason forward as the gate text rather than inventing one, and keeps one age basis for a live and an already-expired hold instead of the expired one restarting the clock.
It publishes only its `resolved ... deferred until <date>` parent line, never a fresh `needs-decision` for the question the captain just postponed.
It requires a non-empty captain decision file of at most 8192 bytes, durably writes a resolution block carrying the decision digest, a `Resolution mode:`, and a `Captain hold answered:` copy of the answered call's hold-set stamp when it has one, while retaining the leading hold-set stamp until the selected `tasks-axi done` or `tasks-axi unhold` transition succeeds, then restores the successful record's resolution-first body ordering (the previous body remains preserved below the block and archived through tasks-axi `--archive-body`).
...
A defer date belongs to the recorded decision's digest, so repeating the same words with a new date is a genuinely new answer that records its own deferral and re-dates the hold, while an exact redelivery stays an idempotent replay.
On a task closed outside the script, `answer` records the missing terminal block only when the captain-hold annotations tasks-axi preserves through a close prove the captain owned it, and it verifies the task stays closed.
A deferred record is non-terminal, so it neither satisfies `verify` after an out-of-band close nor blocks `answer` from recording the later close as a repair.
...
`answers` is its channel-agnostic entry point: it reads `<task-id>\t<answer>\t<label>[\t<mode>[\t<until>]]` lines and resolves each named task through the same `answer` path, so every guard applies identically no matter which channel the answer arrived on.
The optional mode column carries a caller-declared outcome: `done` (default) completes the task, `release` lifts the hold so held work resumes, and `defer` requires a fifth YYYY-MM-DD field strictly later than today's UTC date that records the answer and leaves the task held behind that date; any other value, a missing, past, or same-day defer date, or a date on another mode is skipped.
...
A resolved key is reported `closed:` and counted in `closed=`, except a deferral: it leaves the task queued, held, and dated, so it is reported `deferred: <id> until <date>` and counted separately in `deferred=`, and only `skipped=` still decides the exit status.
...
`bin/fm-send.sh --resolve-key` is the chat channel: its status-log close for a key the status log still owns is owned by that script's header, and a key the status log no longer owns is resolved to a still-open captain-held task - the key as a task id, then the legacy derived identity - and fed as one keyed line; `--defer-until YYYY-MM-DD` selects the dated mode for those captain-held keys and preflights the strict UTC-future boundary before delivery.
It carries that one preflight UTC-day observation into the keyed intake, so a delivery that crosses midnight cannot turn an accepted answer into an unrecorded post-delivery refusal.
...
`bin/fm-procevent-lavish.sh answers` is one such adapter command; it reads only rows tagged `choice`, relays a card's declared close mode or a selected option's own defer date, and can never let freeform captain prose forge a task id, mode, or date.
...
A successful terminal answer also retires any pending request, because an answered call has no remaining re-check obligation.
A deferral leaves the call open and therefore preserves its pending reconcile request for the still-owed re-check.
...
The durable deferral is the recorded answer - `answer --defer-until <date>`, or the keyed intake's `defer` mode with its date - which stores what the captain said alongside the date.
...
Recording a dated defer through `answer --defer-until` or the keyed intake remains the durable fix rather than relying on the projection safety net.
...
It also proves the two verification outcomes - an evidence-backed `reconciled` close that records the evidence under its own label and never as the captain's words, and a note that leaves the call queued, held, and dated - while both outcomes refuse without a pending board request, each durable mutation applies only once across close, probe, and request-retirement failures, a later distinct request with the same note still appends its own dated record, every failed retirement is surfaced with its pending request retained, incompatible resolution modes cannot replay as captain answers, normal close, release, and replay paths retire pending requests, and defer plus its replay preserve them.
The captured-source coverage proves Lavish deduplicates each card before separating versioned structured selections from notes, carries an option-declared defer and date into the shared intake, keeps bare and annotated Reconcile choices out of keyed answers, closes genuine current and legacy choices normally, feeds legacy bare and separator-annotated reconcile values into neither intake, preserves every other card's final value across mixed repeated selections, creates a request through the generic runner only for a verified bound source, creates none from chat reconcile text, and lets the resulting board request authorize evidence-backed closure.
The board's half is pinned in `tests/fm-bearings-board.test.sh`: every published decision card carries exactly one reconcile option, authored options reserve that value across every card type, an option's optional defer date is accepted while an impossible one is refused, recommendations name authored options, a decision card whose structured subject appears in the payload's landed rows is dropped while a genuinely open one is kept even when an unrelated landed id contains its key after a newline, a build requires a fresh authoritative listed-open result before binding or arming, a reopen retires the pre-reopen source generation and waits for a fresh live listener, and a rebuild of an already-armed board with no live listener starts one.
The shipped template's own half is pinned in `tests/fm-bearings-board-render.test.sh`: an option carrying `until` shows the date it commits the call to on the card and in the queued confirmation and emits the dated defer answer context over any card close, while an option without one shows no date and leaves the card's close governing.
...
The exact commands and their summarized outputs are recorded in the shipping PR's evidence; run the four suites above plus `tests/fm-send-resolve-key.test.sh`, `tests/fm-bearings-board.test.sh`, `tests/fm-bearings-board-render.test.sh`, `tests/fm-procevent.test.sh`, and `bin/fm-lint.sh` to refresh this record, and `FM_BEARINGS_LAVISH_LIVE=1 tests/fm-bearings-board-lavish-live-e2e.test.sh` after a lavish-axi upgrade.
