...
A deferral continues the call it answers rather than opening another, so it leaves the `Captain hold set:` stamp alone - including preserving its absence on a legacy stampless hold - and a live and an already-expired hold keep one age basis instead of the expired one restarting the clock.
