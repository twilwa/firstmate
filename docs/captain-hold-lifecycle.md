...
The `answer` subcommand records the captain's exact words and resolves the call in the same act: it closes a question-shaped call, `answer --release` frees a captain-gated work item to proceed without completing it, and `answer --defer-until YYYY-MM-DD` routes through the existing `hold --until` path so the answered call remains held behind its date.
