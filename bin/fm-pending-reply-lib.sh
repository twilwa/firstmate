...
fm_pending_reply_tick() {  # <state-dir>
  local state=$1 dir rec corr task_id phase escalated field delivered meta backend target label busy sm_home harness remote_host
  ...
  for rec in "$dir"/*; do
    ...
    if [ "$phase" = resolved ]; then
      # Only an escalated reply can have an open decision to close. Still retry
      # those closes after transient failures, including on later cycles.
      [ -z "$escalated" ] || fm_pending_reply_close_escalation "$state" "$corr" || true
      continue
    fi
    fm_pending_reply_reconcile_delivery "$state" "$corr" || true
    phase=$(fm_pending_reply_get "$rec" phase)
