...
- `bin/fm-pr-review.sh merge` is the only GitHub merge entrypoint and requires the current head's review-ledger generation; `bin/fm-pr-merge.sh` remains the direct GitLab merge entrypoint.
  A merge proceeds only under the same authority MAIN would have attended, when the pull request is green at its live reviewed head, synchronously under the record lock; a red pull request is never merged while away, whatever the words say, and `--allow-red` is refused under the record.
