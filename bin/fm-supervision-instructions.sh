...
repair_line() {
  ...
  case "$HARNESS" in
    claude)
      ...
      ;;
    codex)
      printf '%s%s\n' "$prefix" 'the Codex Stop-hook watcher park failed to retain a callback; inspect bin/fm-codex-stop-park.sh and its hook registration before ending the turn.'
      ;;
    pi|pi-signed)
...
ordinary_wake_line() {
  case "$HARNESS" in
    claude)
      ...
      ;;
    codex)
      printf '%s\n' '- Ordinary wake: the Stop-hook park (bin/fm-codex-stop-park.sh) already owns watcher continuity; drain and handle the wake, and let the next turn end park again.'
      ;;
    pi|pi-signed)
